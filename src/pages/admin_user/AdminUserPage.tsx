...
import CustomerPurchaseOrder from "../../customers/CustomerPurchaseOrder"
import CustomerPayments from "../../customers/CustomerPayments"
import CustomerChequeManagement from "../../customers/CustomerChequeManagement"
import CustomerCreditNotes from "../../customers/CustomerCreditNotes"
import CustomerCreditNoteSettlements from "../../customers/CustomerCreditNoteSettlements"
  ...
  return (
    <div className="flex h-full flex-col items-start justify-center gap-2 rounded-xl border border-dashed bg-background px-8 py-16">
      ...
      <h2 className="text-xl font-semibold">
        {meta?.label ?? dynamicLabel ?? "Untitled page"}
      </h2>
      <p className="max-w-md text-sm text-muted-foreground">
        ...
        with the real component once it's built.
      </p>
      {Object.keys(params).length > 0 && (
        <pre className="mt-2 rounded-lg bg-muted px-3 py-2 text-[11px]">
...
const STATIC_ROUTE_PATHS: string[] = Object.values(ADMIN_ROUTES).filter(
...
const ADMIN_ROUTE_COMPONENTS: Partial<Record<string, React.ComponentType>> = {
  ...
  [ADMIN_ROUTES.CUSTOMERS_ALL]: AllCustomers,
  [ADMIN_ROUTES.CUSTOMERS_PENDING]: PendingCustomers,
  [ADMIN_ROUTES.CUSTOMERS_PURCHASE_ORDERS]: CustomerPurchaseOrder,
  [ADMIN_ROUTES.CUSTOMERS_PAYMENTS]: CustomerPayments,
  [ADMIN_ROUTES.CUSTOMERS_CHEQUE_MANAGEMENT]: CustomerChequeManagement,
  [ADMIN_ROUTES.CUSTOMERS_CREDIT_NOTE]: CustomerCreditNotes,
  [ADMIN_ROUTES.CUSTOMERS_CREDIT_NOTE_SETTLEMENTS]:
    CustomerCreditNoteSettlements,
  [ADMIN_ROUTES.CUSTOMERS_REPORTS]: CustomerReports,
          ...
          <AdminLayout>
            <Routes>
              {STATIC_ROUTE_PATHS.map((routePath) => {
                ...
                const relativePath =
                  routePath.replace(ADMIN_ROUTES.ADMIN, "") || "/"
                  ...
                  <Route
                    ...
                    element={
                      RealComponent ? (
                        ...
                        <PlaceholderPage routePath={routePath} />
                      )
                    }
                  />
                )
              ...
              <Route
                path={ADMIN_ROUTES.INVOICE_DRAFT.replace(
                  `${ADMIN_ROUTES.ADMIN}/`,
                  ""
                )}
                element={
                  <PlaceholderPage
                    routePath={ADMIN_ROUTES.INVOICE_DRAFT}
                    dynamicLabel="Invoice Draft"
                  />
                }
              />
              <Route
                path={ADMIN_ROUTES.CUSTOMERS_PURCHASE_ORDERS_EDIT.replace(
                  `${ADMIN_ROUTES.ADMIN}/`,
                  ""
                )}
                element={
              ...
              <Route
                ...
                element={
                  <PlaceholderPage
                    routePath={ADMIN_ROUTES.ACCOUNTS_DASHBOARD}
                    dynamicLabel="Dashboard"
                  />
                }
              />
            </Routes>
